...
When the last track of an album plays, the app automatically increments a Play Count custom field in your Discogs collection and optionally records a Last Played date. When Last.fm scrobbling is enabled (opt-in), each identified track is scrobbled on a best-effort basis: a transient Last.fm failure is retried a few times in memory, but scrobbling is not guaranteed and is never persisted across restarts.
